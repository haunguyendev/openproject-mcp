...
- **Dynamic Client Registration (DCR)** — set `OP_OAUTH_CLIENT_ID` to advertise a `registration_endpoint`; `POST /oauth/register` returns the pre-configured OpenProject client (RFC 7591) so Claude.ai auto-registers and users connect **without pasting a client_id/secret**. A **public** client (no secret) is recommended (`token_endpoint_auth_method: none`, PKCE); a confidential client is supported but warns at startup since the open endpoint would serve its secret. When unset, the manual-paste flow is unchanged.
...
- DNS-rebinding/Origin protection in http mode (`TransportSecuritySettings`). New env vars: `MCP_TRANSPORT`, `MCP_HOST`, `MCP_PORT`, `MCP_PUBLIC_URL`, `ALLOWED_HOSTS`, `ALLOWED_ORIGINS`, `OP_MCP_ENABLE_ADMIN_DESTRUCTIVE`, `OP_OAUTH_CLIENT_ID`, `OP_OAUTH_CLIENT_SECRET`.
- Deployment artifacts: `Dockerfile`, `docker-compose.yml`, `.env.example`, `deploy/caddy.snippet`, `deploy/nginx.snippet`, and Kubernetes manifests `deploy/k8s/openproject-mcp.yaml` (Namespace, ConfigMap, Deployment, Service, Ingress — public OAuth means no Kubernetes Secrets; stateless and horizontally scalable). Deployment guide updated with the remote/OAuth/DCR onboarding and a Kubernetes section.
